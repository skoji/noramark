...
describe ArtiMark do 
  describe 'convert' do
    it 'should generate valid xhtml' do
      text = 'some text'
      artimark = ArtiMark::Document.open(text, :lang => 'ja', :title => 'the title')
    ...
    it 'should convert simple paragraph in english mode' do
      ...
      artimark = ArtiMark::Document.open(text, :lang => 'en', :title => 'the title')
      converted = artimark.html
      body = Nokogiri::XML::Document.parse(converted[0]).root.at_xpath('xmlns:body')
      expect(body.element_children.size).to eq 2
      expect(body.element_children[0].selector_and_children).to eq(
    ...
    it 'should convert simple paragraph in japanese mode, but paragraph mode is default' do
      text = "paragraph begins.\n2nd line.\n 3rd line.\n\n\n next paragraph."
      artimark = ArtiMark::Document.open(text, :lang => 'ja', :title => 'the title', :paragraph_style => :default)
      converted = artimark.html
      body = Nokogiri::XML::Document.parse(converted[0]).root.at_xpath('xmlns:body')
      expect(body.element_children.size).to eq 2
      expect(body.element_children[0].selector_and_children).to eq(
    ...
    it 'should convert div and paragraph' do
      text = "d {\n1st line. \n}"
      artimark = ArtiMark::Document.open(text, :lang => 'ja', :title => 'the document title')
      converted = artimark.html
      body = Nokogiri::XML::Document.parse(converted[0]).root.at_xpath('xmlns:body')
      expect(body.element_children[0].selector_and_children).to eq(
        ['div',
    ...
    it 'should handle stylesheets' do
      ...
      artimark = ArtiMark::Document.open(text, :lang => 'ja', :title => 'the document title', :stylesheets => ['reset.css', 'mystyle.css'])
      converted = artimark.html
    ...
    it 'should specify title on each page' do
      text = "title:page1\n\n1st page.\nnewpage:\ntitle:page2\nh1:2nd page"
      artimark = ArtiMark::Document.open(text, :lang => 'ja', :title => 'the title', :paragraph_style => :use_paragraph_group)
      converted = artimark.html
      # 1st page
      head = Nokogiri::XML::Document.parse(converted[0]).root.at_xpath('xmlns:head')
    ...
    it 'should handle preprocessor' do
      ...
      artimark = ArtiMark::Document.open(text, :lang => 'ja', :title => 'the title') do
        |arti|
...
EOF
    ...
    it 'should raise error' do
      text = "d {\n block is\nd {\n nested but\nd {\n not terminated }"
      expect { ArtiMark::Document.open(text, :lang => 'ja', :title => 'foo') }.to raise_error KPeg::CompiledParser::ParseError
    end
    ...
    describe 'create file' do
      ...
      it 'should create named file' do
        text = "some text"
        artimark = ArtiMark::Document.open(text, :lang => 'ja', :title => 'the document title', filename_prefix: 'nora-test-file' )
        artimark.html.write_as_files(directory: @basedir)
